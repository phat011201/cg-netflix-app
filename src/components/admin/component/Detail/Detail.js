...
function Detail() {
  ...
  useEffect(() => {
    axios
      .get(`${MOVIES_API_URL}videos/${movieId}`)
  ...
  const handleClickDelete = () => {
    axios
      .delete(`${MOVIES_API_URL}videos/${movieId}`)
      .then(() => {
        navigate("/admin");
      })
      .catch((err) => {
  ...
  return (
    <>
      <div className={cx("wrapper")}>
        <div className={cx("item")}>Movie ID: {data.id}</div>
        ...
        <div className={cx("item")}>genre: {data.genre}</div>
        <div className={cx("item")}>duration: {data.duration}</div>
